...
const getVariantDetails = async (options: Options[]): Promise<VariantDetails> => {
    const lookupKey = ["Color", "RAM", "Storage"].reduce(
        (acc, x) => `${acc}${x}${findTypeValue(options, x)}`,
        ""
    );
    const variantDetails: VariantDetails = (await axios.get(`${baseUrl}/product-variants/${lookupKey}`)).data
